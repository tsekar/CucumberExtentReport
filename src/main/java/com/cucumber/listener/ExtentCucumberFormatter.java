...
public class ExtentCucumberFormatter implements Reporter, Formatter {
	...
	/**
	 * When running cucumber tests in parallel Klov reporter should be attached
	 * only once, in order to avoid duplicate builds on klov server.
	 */
	private static synchronized void setKlovReport() {
		...
		if (klovReporter != null) {
			// Already attached, attaching it again will create a new build/klov
			// report
			return;
		}
	...
	public void result(Result result) {
		...
		if (Result.PASSED.equals(result.getStatus())) {
			...
			String fileName = s + ".jpeg";

			String path = "D:/PVH/Screenshot";
			
			try {
				takeScreenshot(path, fileName, com.cucumber.stepdefinitions.Base.driver);
				com.cucumber.listener.Reporter.addScreenCaptureFromPath(path  +"/" +fileName, "PASS");
	...
	public void takeScreenshot(String path, String fileName, WebDriver driver) throws IOException {
